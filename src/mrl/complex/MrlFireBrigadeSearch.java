...
import mrl.complex.firebrigade.ExploreManager;
import mrl.complex.firebrigade.MrlFireBrigadeWorld;
import mrl.complex.firebrigade.TargetBaseExploreManager;
...
//        exploreManager = new TargetBaseExploreManager(ai, wi, si, moduleManager, developData);
    ...
    private EntityID search(EntityID lastTarget, Set<EntityID> searchableEntityIds, boolean sorted) {
        ...
        if (searchableEntityIds != null && !searchableEntityIds.isEmpty()) {
            if (lastTarget != null && searchableEntityIds.contains(lastTarget)) {//上次搜寻未完成继续上次。
                if (hasPath(lastTarget)) {
                    return lastTarget;
                } else {
                    addToUnreachableTargets(lastTarget);//上次搜寻未完成，且无法到达。
                }
            }
